...
fn print_rawframepacket(packet: &RawFramePacket) {
    ...
    let size_match = if packet.data.len == expected_size { "✓" } else { "✗" };
    println!("║   expected_size  : {} bytes {}", expected_size, size_match);
    ...
    if !packet.data.ptr.is_null() && packet.data.len > 0 {
        ...
        let sample = unsafe {
            std::slice::from_raw_parts(packet.data.ptr, sample_size)
        };
...
fn verify_packet(packet: &RawFramePacket) {
    ...
    match packet.meta.pixfmt {
        ...
        _ => {
            println!("║   [✗] PixelFormat is {:?} (expected YUV422_8)", packet.meta.pixfmt);
            all_ok = false;
        }
    }
    ...
    match packet.meta.colorspace {
        ...
        _ => {
            println!("║   [✗] ColorSpace is {:?} (expected BT709)", packet.meta.colorspace);
            all_ok = false;
        }
    }
    ...
    match packet.data.loc {
        ...
        _ => {
            println!("║   [✗] Data location is {:?} (expected CPU)", packet.data.loc);
            all_ok = false;
        }
    }
    ...
    if packet.meta.width > 0 && packet.meta.height > 0 {
        println!("║   [✓] Dimensions are valid ({}x{})", packet.meta.width, packet.meta.height);
    } else {
        println!("║   [✗] Invalid dimensions: {}x{}", packet.meta.width, packet.meta.height);
        all_ok = false;
    }
    ...
    if packet.meta.stride_bytes >= min_stride {
        println!("║   [✓] Stride is valid ({} >= {} min)", packet.meta.stride_bytes, min_stride);
    } else {
        println!("║   [✗] Stride too small ({} < {} min)", packet.meta.stride_bytes, min_stride);
        all_ok = false;
    }
    ...
    if packet.data.len == expected {
        ...
        println!("║   [✗] Data size mismatch ({} != {})", packet.data.len, expected);
        all_ok = false;
    }
