...
fn main() {
    ...
    let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        fragment: Some(wgpu::FragmentState { module: &shader, entry_point: "fs_main", targets: &[Some(format.into())] }),
    ...
    event_loop.run(|event, elwt| {
        match event {
            ...
            Event::AboutToWait => {
                    ...
                    if cf.seq != 0 && cf.seq != last_seq && !cf.data.is_null() && cf.width > 0 && cf.height > 0 {
                        ...
                        if v_width != w || v_height != h || video_tex.is_none() {
                            ...
                            let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
                                ...
                                entries: &[
                                    wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::Sampler(bgl_sampler) },
                        ...
                        if let Some(tex) = &video_tex {
                            ...
                            let src = unsafe { std::slice::from_raw_parts(cf.data, (rb) * (h as usize)) };
...

                {
                    let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        ...
                        color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                            view: &view,
                            resolve_target: None,
                            ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color { r: 0.125, g: 0.125, b: 0.125, a: 1.0 }), store: wgpu::StoreOp::Store },
                        })],
                ...
                frames += 1;
                if last_fps.elapsed() >= Duration::from_secs(1) { println!("fps: {}", frames); frames = 0; last_fps = Instant::now(); }
            }
            _ => {}
        }
    }).expect("run");
