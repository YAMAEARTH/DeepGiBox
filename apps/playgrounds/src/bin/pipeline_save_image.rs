//! Pipeline: Capture → Overlay → Save as Image
//!
//! This pipeline captures frames, runs inference, creates overlays,
//! and saves the result as PNG images for inspection.
...
fn main() -> Result<()> {
    println!("╔══════════════════════════════════════════════════════════╗");
    println!("║  PIPELINE: CAPTURE → OVERLAY → SAVE IMAGE               ║");
    println!("║  Save detection results as PNG images                   ║");
    println!("╚══════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!("🎬 Step 7: Processing Frames...");
    println!("════════════════════════════════════════════════════════════");
    println!();
    ...
    while saved_count < frames_to_save {
        ...
        // Store original frame dimensions and data for image saving
        ...
        // Copy frame data to CPU if on GPU (for later image conversion)
        ...
        // Convert YUV422 to RGB for image saving
        let rgb_image = yuv422_to_rgb(&cpu_frame_data, orig_width, orig_height, orig_stride);

        // Draw overlay on RGB image
        let mut rgba_image = vec![0u8; (orig_width * orig_height * 4) as usize];
        for y in 0..orig_height {
            for x in 0..orig_width {
                let rgb_idx = ((y * orig_width + x) * 3) as usize;
                let rgba_idx = ((y * orig_width + x) * 4) as usize;
                rgba_image[rgba_idx] = rgb_image[rgb_idx];
                rgba_image[rgba_idx + 1] = rgb_image[rgb_idx + 1];
                rgba_image[rgba_idx + 2] = rgb_image[rgb_idx + 2];
                rgba_image[rgba_idx + 3] = 255;
            }
        }
        ...
        for op in &overlay_plan.ops {
            match op {
                common_io::DrawOp::Rect { xywh, thickness, color } => {
                    let (x, y, w, h) = *xywh;
                    let (a, r, g, b) = *color;
                    draw_rect_on_rgba(&mut rgba_image, orig_width, orig_height,
                                     x as i32, y as i32, w as i32, h as i32,
                                     r, g, b, a, *thickness as i32);
                }
                common_io::DrawOp::Label { anchor, text, color, .. } => {
                    let (x, y) = *anchor;
                    let (a, r, g, b) = *color;
                    draw_label_on_rgba(&mut rgba_image, orig_width, orig_height,
                                       x as i32, y as i32, text, r, g, b, a);
                }
                _ => {}
            }
        }
        ...
        save_rgba_as_png(&rgba_image, orig_width, orig_height, &output_path)?;
...
// Draw rectangle on RGBA image
fn draw_rect_on_rgba(img: &mut [u8], width: u32, height: u32, x: i32, y: i32, w: i32, h: i32,
                     r: u8, g: u8, b: u8, a: u8, thickness: i32) {
        ...
        for dx in 0..w {
            ...
            if px >= 0 && px < width as i32 {
                ...
                if py_top >= 0 && py_top < height as i32 {
                    set_pixel_rgba(img, width, px, py_top, r, g, b, a);
                }
                let py_bottom = y + h - 1 - t;
                if py_bottom >= 0 && py_bottom < height as i32 {
                    set_pixel_rgba(img, width, px, py_bottom, r, g, b, a);
                }
            }
        }
        ...
        for dy in 0..h {
            ...
            if py >= 0 && py < height as i32 {
                ...
                if px_left >= 0 && px_left < width as i32 {
                    set_pixel_rgba(img, width, px_left, py, r, g, b, a);
                }
                let px_right = x + w - 1 - t;
                if px_right >= 0 && px_right < width as i32 {
                    set_pixel_rgba(img, width, px_right, py, r, g, b, a);
                }
            }
        }
    }
...
// Draw label on RGBA image
fn draw_label_on_rgba(img: &mut [u8], width: u32, height: u32, x: i32, y: i32, 
                      text: &str, r: u8, g: u8, b: u8, a: u8) {
    ...
    for dy in 0..label_height {
        for dx in 0..label_width {
            let px = x + dx;
            let py = y - label_height + dy;
            if px >= 0 && px < width as i32 && py >= 0 && py < height as i32 {
                set_pixel_rgba(img, width, px, py, 0, 0, 0, 200);
            }
        }
    }
    
    // Draw text marker (simple colored bar)
    for dy in 5..15 {
        for dx in 5..(5 + text.len() as i32 * 6) {
            let px = x + dx;
            let py = y - label_height + dy;
            if px >= 0 && px < width as i32 && py >= 0 && py < height as i32 {
                set_pixel_rgba(img, width, px, py, r, g, b, a);
            }
        }
    }
}

fn set_pixel_rgba(img: &mut [u8], width: u32, x: i32, y: i32, r: u8, g: u8, b: u8, a: u8) {
...
fn save_rgba_as_png(data: &[u8], width: u32, height: u32, path: &str) -> Result<()> {
    use std::fs::File;
