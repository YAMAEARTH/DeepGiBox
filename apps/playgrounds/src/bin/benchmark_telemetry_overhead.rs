...
fn main() {
    ...
    println!(
        "Frequency:       {:.2} MHz (calls per second)",
        1000.0 / baseline_per_call
    );
    println!();
    ...
    println!(
        "vs Baseline:     {:.2}x",
        elapsed_per_call / baseline_per_call
    );
    println!();
    ...
    println!(
        "Total time:      {:.3} ms",
        now_ns_total.as_secs_f64() * 1000.0
    );
    println!("Per call:        {:.3} ns", now_ns_per_call);
    println!(
        "vs Baseline:     {:.2}x",
        now_ns_per_call / baseline_per_call
    );
    println!();
    ...
    println!(
        "Total time:      {:.3} ms",
        since_ms_total.as_secs_f64() * 1000.0
    );
    println!("Per call:        {:.3} ns", since_ms_per_call);
    println!(
        "vs Baseline:     {:.2}x",
        since_ms_per_call / baseline_per_call
    );
    println!();
    ...
    println!("─────────────────────────────────────────────────────────────");
    println!(
        "Running {} iterations (this will print to stderr)...",
        ITERATIONS / 1000
    );

    let start = Instant::now();
    for i in 0..(ITERATIONS / 1000) {
        // Reduced iterations to avoid spamming
    ...
    println!(
        "\nTotal time:      {:.3} ms",
        record_ms_total.as_secs_f64() * 1000.0
    );
    println!("Per call:        {:.3} ns", record_ms_per_call);
    println!(
        "vs Baseline:     {:.2}x",
        record_ms_per_call / baseline_per_call
    );
    ...
    println!(
        "Without telemetry: {:.3} ms",
        without_telemetry.as_secs_f64() * 1000.0
    );
    println!(
        "With telemetry:    {:.3} ms",
        with_telemetry.as_secs_f64() * 1000.0
    );
    ...
    println!("├────────────────────────────────┼──────────────┼───────────────┤");
    println!(
        "│ std::time::Instant::now()      │ {:>12.2} │      1.00x    │",
        baseline_per_call
    );
    println!(
        "│ Instant::elapsed()             │ {:>12.2} │ {:>10.2}x    │",
        elapsed_per_call,
        elapsed_per_call / baseline_per_call
    );
    println!(
        "│ telemetry::now_ns()            │ {:>12.2} │ {:>10.2}x    │",
        now_ns_per_call,
        now_ns_per_call / baseline_per_call
    );
    println!(
        "│ telemetry::since_ms()          │ {:>12.2} │ {:>10.2}x    │",
        since_ms_per_call,
        since_ms_per_call / baseline_per_call
    );
    println!(
        "│ telemetry::record_ms() (I/O)   │ {:>12.2} │ {:>10.2}x    │",
        record_ms_per_call,
        record_ms_per_call / baseline_per_call
    );
    println!("└────────────────────────────────┴──────────────┴───────────────┘");
    ...
    println!(
        "   ~{:.2} ns per measurement",
        now_ns_per_call + since_ms_per_call
    );
    println!(
        "   = {:.6} ms per measurement",
        (now_ns_per_call + since_ms_per_call) / 1_000_000.0
    );
    println!();
    ...
    println!(
        "   • Disable logging in production or log to buffer: ~{:.2} ns overhead",
        now_ns_per_call + since_ms_per_call
    );
    println!(
        "   • Keep stderr logging for development: ~{:.2} ns overhead",
        record_ms_per_call
    );
    println!();
