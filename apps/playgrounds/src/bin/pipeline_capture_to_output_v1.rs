...
fn main() -> Result<()> {
    ...
    let (width, height) = loop {
        if let Some(frame) = capture.get_frame()? {
            ...
            if w >= 1920 && h >= 1080 {
                println!("  ✓ Got frame dimensions: {}x{}", w, h);
                break (w, h);
            }
        }
    ...
    loop {  // Changed to infinite loop
        ...
        println!();
        println!("🔑 Step 8: Internal Keying");
        let keying_start = Instant::now();
        
        // Convert overlay ARGB → BGRA for GPU compositor
        let bgra_overlay = overlay_to_bgra(&overlay_frame)?;
        
        // Update GPU compositor with new overlay
        gpu_compositor = OutputSession::new(width, height, &bgra_overlay)?;
        
        // Composite overlay on background using CUDA kernel
        ...
        use common_io::{PixelFormat, ColorSpace};
        ...
        println!("  ✓ ARGB → BGRA conversion");
        println!("  ✓ GPU Composite (CUDA kernel)");
        println!("  ✓ DeckLink output submitted");
        println!("  ✓ Pipeline: Background (GPU) + Overlay (BGRA) → DeckLink SDI");

        // Step 9: Save images (original, overlay, and comparison)
        println!();
        println!("💾 Step 9: Save Images");
        ...
        // Print average stats every 60 frames
        if frame_count > 0 && frame_count % 60 == 0 {
            let elapsed_secs = pipeline_start_time.elapsed().as_secs_f64();
            let avg_fps = frame_count as f64 / elapsed_secs;
            let avg_latency = total_latency_ms / frame_count as f64;
            
            println!();
            println!("════════════════════════════════════════════════════════════");
            println!("📊 Stats after {} frames ({:.1}s elapsed):", frame_count, elapsed_secs);
            println!("  Average FPS: {:.2}", avg_fps);
            println!("  Average E2E Latency: {:.2}ms", avg_latency);
            println!("  Breakdown:");
            println!("    Capture:      {:.2}ms", total_capture_ms / frame_count as f64);
            println!("    Preprocess:   {:.2}ms", total_preprocess_ms / frame_count as f64);
            println!("    Inference:    {:.2}ms", total_inference_ms / frame_count as f64);
            println!("    Postprocess:  {:.2}ms", total_postprocess_ms / frame_count as f64);
            println!("    Overlay Plan: {:.2}ms", total_plan_ms / frame_count as f64);
            println!("    Overlay Render:{:.2}ms", total_render_ms / frame_count as f64);
            println!("    Internal Key: {:.2}ms", total_keying_ms / frame_count as f64);
            println!("════════════════════════════════════════════════════════════");
            println!();
        }
        println!();
        ...
        // Small delay removed for max speed
        // std::thread::sleep(std::time::Duration::from_millis(16));
    }
