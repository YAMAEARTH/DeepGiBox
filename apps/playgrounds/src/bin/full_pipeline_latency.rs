...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("   ┃   FROM:      System RAM ({:p})                                           ┃", packet.data.ptr);
    ...
    println!("   • Size: {:.2} MB → Takes {:.2}-{:.2} ms", frame_size_mb, h2d_gen4, h2d_gen3);
    println!("   • นี่คือ overhead ที่หลีกเลี่ยงไม่ได้");
    
    println!("\n2. WHY THIS MATTERS");
    println!("   • At 60fps: frame budget = 16.67ms");
    println!("   • H2D alone: {:.2} ms (PCIe 3.0) = {:.1}% of budget!", h2d_gen3, h2d_gen3 / 16.67 * 100.0);
    println!("   • H2D alone: {:.2} ms (PCIe 4.0) = {:.1}% of budget", h2d_gen4, h2d_gen4 / 16.67 * 100.0);
    ...
    println!("   Step 4: cudaMemcpy() to GPU ⚠️          (MANDATORY, ~{:.2}ms)", h2d_gen3);
    ...
    println!("   • Example: 1920×1080 → 1280×720 = save {:.2}ms", 
