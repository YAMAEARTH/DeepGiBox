    ...
    for attempt in 0..300 {
        ...
        if let Some(packet) = packet_option {
            ...
            if is_valid && buffer_size >= 8 {
                ...
                let _sample_data = unsafe {
                    std::slice::from_raw_parts(packet.data.ptr, 8)
                };
            }
            ...
            if frame_count % 10 == 0 {
                println!("Frame #{}: {}x{} stride={} size={} bytes pixfmt={:?} colorspace={:?}",
        ...
        if attempt >= 299 {
            println!("\nWarning: Reached maximum attempts (300) with only {} frames captured", frame_count);
            break;
        }
    }
