...
fn main() -> Result<()> {
    ...
    while frame_count < frames_to_process {
        ...
        // COMMENTED OUT: Step 5: Postprocessing
        // Instead, we'll create DetectionsPacket directly from top 5 raw detections
        /*
        println!();
        ...
        */
        ...
        // Step 5: Extract top 5 detections directly from inference (BYPASS POSTPROCESSING)
        println!();
        println!("🎯 Step 5: Extract Top 5 Detections from Raw Inference (NO POSTPROCESSING)");
        let postprocess_start = Instant::now();
        ...
        // Take top 5 and convert to DetectionsPacket
        let mut detection_items = Vec::new();
        for (rank, (_, x, y, w, h, obj_conf, class0_conf, class1_conf)) in detections_with_scores.iter().take(5).enumerate() {
            // Determine class (0=Hyper, 1=Neo) based on which has higher confidence
            let (class_id, class_conf) = if class0_conf > class1_conf {
                (0, *class0_conf)
            } else {
                (1, *class1_conf)
            };
            
            let final_score = obj_conf * class_conf;
            
            detection_items.push(common_io::Detection {
                class_id,
                score: final_score,
                bbox: common_io::BBox {
                    x: *x,
                ...
                track_id: Some(rank as i32), // Use rank as fake track ID
            });
        }
        
        let detections = common_io::DetectionsPacket {
            from: raw_detections.from.clone(),
        ...
        println!("      → Operations: Decode YOLO output, NMS (IoU=0.45), SORT tracking");
