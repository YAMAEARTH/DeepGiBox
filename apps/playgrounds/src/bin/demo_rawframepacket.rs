use decklink_input::{ColorSpace, FrameMeta, MemLoc, MemRef, PixelFormat, RawFramePacket};
...
fn verify_packet_structure(packet: &RawFramePacket) {
    ...
    if matches!(packet.meta.pixfmt, PixelFormat::YUV422_8) {
        ...
        println!(
            "✗ [1] PixelFormat is {:?}, expected YUV422_8",
            packet.meta.pixfmt
        );
    }
    ...
    if matches!(packet.meta.colorspace, ColorSpace::BT709) {
        ...
        println!(
            "✗ [2] ColorSpace is {:?}, expected BT709",
    ...
    if packet.meta.width > 0 && packet.meta.height > 0 {
        println!(
            "✓ [3] Dimensions are valid: {}x{}",
            packet.meta.width, packet.meta.height
        );
        checks_passed += 1;
    } else {
        println!(
            "✗ [3] Invalid dimensions: {}x{}",
            packet.meta.width, packet.meta.height
        );
    }
    ...
    if packet.meta.stride_bytes >= min_stride {
        println!(
            "✓ [4] Stride is valid: {} bytes (>= {} minimum)",
    ...
    if packet.data.len == expected_size {
        println!(
            "✓ [5] Data size matches: {} bytes (stride × height)",
            packet.data.len
        );
        checks_passed += 1;
    } else {
        println!(
            "✗ [5] Data size mismatch: {} != {}",
            packet.data.len, expected_size
        );
    }
    ...
    if checks_passed == checks_total {
        println!(
            "│ ✓✓✓ All {} checks PASSED! Packet is pipeline-ready!  │",
            checks_total
        );
    } else {
        println!(
