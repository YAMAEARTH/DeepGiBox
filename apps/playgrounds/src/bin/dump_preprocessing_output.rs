...
fn dump_tensor_to_file(
    ...
    match tensor.desc.dtype {
        DType::Fp32 => {
            ...
            writeln!(txt_file, "Tensor Shape: {}×{}×{}×{} (NCHW)", 
        ...
        DType::Fp16 => {
            ...
            let bytes = unsafe {
                std::slice::from_raw_parts(
                    host_data_u16.as_ptr() as *const u8,
                    total_elements * 2
                )
            };
            file.write_all(bytes)?;
            ...
            use half::f16;
            let host_data_f32: Vec<f32> = host_data_u16.iter()
...
fn dump_raw_frame_info(raw: &RawFramePacket) {
    ...
    println!("║  Dimensions:    {}×{}{:>37} ║", raw.meta.width, raw.meta.height, "");
    println!("║  Pixel Format:  {:?}{:>32} ║", raw.meta.pixfmt, "");
    println!("║  Color Space:   {:?}{:>34} ║", raw.meta.colorspace, "");
    println!("║  Stride:        {} bytes{:>36} ║", raw.meta.stride_bytes, "");
    println!("║  Data Length:   {} bytes ({:.2} MB){:>23} ║", 
             raw.data.len, 
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("  Output Size:   {}×{}", preprocessor.size.0, preprocessor.size.1);
            ...
            Some(frame) => {
                ...
                if matches!(frame.data.loc, MemLoc::Gpu { .. }) {
                    ...
                    if is_stable && gpu_frame_count >= MIN_STABLE_FRAMES {
                        println!("  ✓ Stable GPU frame captured! ({}×{}, frame #{})", 
                                 frame.meta.width, frame.meta.height, gpu_frame_count);
                        break frame;
                    } else if is_stable {
                        println!("  ⏭️  Skipping GPU frame #{}/{} (waiting for stable frames)...", 
                                 gpu_frame_count, MIN_STABLE_FRAMES);
                    } else {
                        println!("  ⚠️  Unexpected GPU frame size: {}×{} (frame #{})", 
                                 frame.meta.width, frame.meta.height, gpu_frame_count);
                    }
                } else {
                    println!("  ⏭️  Skipping CPU frame ({}×{})...", frame.meta.width, frame.meta.height);
                }
            }
            None => {
    ...
    println!("   data = np.fromfile('{}_fp16.bin', dtype=np.float16)", output_prefix);
