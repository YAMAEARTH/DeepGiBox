...
pub fn run_gpu_inference(packet: &TensorInputPacket) -> Vec<f32> {
    unsafe {
        ...
        // Run inference (input is already on GPU!)
        ...
        println!("\n⏱️  Performance:");
        println!("   ┌──────────────────────┬──────────┐");
        println!("   │ GPU Inference        │ {:>6.2}ms │", inference_time.as_secs_f64() * 1000.0);
        println!("   │ Copy GPU → CPU       │ {:>6.2}ms │", copy_time.as_secs_f64() * 1000.0);
        println!("   ├──────────────────────┼──────────┤");
        let total = inference_time.as_secs_f64() * 1000.0 + copy_time.as_secs_f64() * 1000.0;
